...
import geoCoordHubMap from "../assets/geoCoordHubMap";

let mapPoints = [];
for (const hub in geoCoordHubMap) {
  if (geoCoordHubMap.hasOwnProperty(hub)) {
    const coords = geoCoordHubMap[hub];
    mapPoints.push({ name: hub, value: coords });
  }
}
export default {
  name: "myMap4",
  ...
  methods: {
    drawMap() {
      ...
      that.setFitView(
        !that.lines.length
          ? that.mapPoints.map(point => {
              return point.value;
            })
          : that.getLinePointCoords()
      );
    },
    getLinePointCoords() {
      let coords = [];
      this.mapData.forEach(points => {
        points.forEach(point => {
          coords.push(geoCoordHubMap[point]);
        });
      });
    ...
    convertPoint2Line() {
      let that = this;
      let lines = [];
      lines = that.mapData.map(points => {
        return {
          coords: points.map(point => {
            return geoCoordHubMap[point];
          })
        };
      });
      that.lines = lines;
